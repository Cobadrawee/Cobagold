/**
 * Hourly sync: gold spot -> setUsdtMicroPerGram on COBA contract (owner wallet).
 * Secrets: GOLD_NFT_SYNC_PRIVATE_KEY, GOLD_NFT_CONTRACT, optional ETHEREUM_RPC_URL, METALAPI_KEY
 */
...
async function main() {
  ...
  const raw = process.env.GOLD_NFT_CONTRACT?.trim() ?? ''
  if (!/^0x[a-fA-F0-9]{40}$/.test(raw)) {
    console.error('Missing GOLD_NFT_CONTRACT')
    process.exit(1)
  }

  const rpcUrl = process.env.ETHEREUM_RPC_URL?.trim() || 'https://eth.llamarpc.com'
  const metalKey = process.env.METALAPI_KEY?.trim() || ''
